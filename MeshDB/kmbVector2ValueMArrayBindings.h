...
class Vector2ValueMArrayBindings : public kmb::DataBindings
{
private:
	BLArray<double,2> vArray;
	BLArrayIndex aIndex;
	size_t count;
public:
	Vector2ValueMArrayBindings(size_t count,kmb::DataBindings::bindingMode bmode=kmb::DataBindings::NODEVARIABLE);
	virtual ~Vector2ValueMArrayBindings(void);
	virtual const char* getContainerType(void) const{
		return "Vector2ValueMArrayBindings";
	};
...
public:
	...
	{
		friend class Vector2ValueMArrayBindings;
	...
	private:
		const kmb::Vector2ValueMArrayBindings* values;
