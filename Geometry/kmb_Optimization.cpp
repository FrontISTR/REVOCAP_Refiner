...
int kmb::Optimization::maxIter = 1000;
double kmb::Optimization::epsilon = 1.0e-8;
...
int
kmb::Optimization::getIterCount(void)
{
	return iterCount;
}

double
kmb::Optimization::getResidual(void)
{
	return residual;
}
...
kmb::Optimization::calcZero_NR( FuncSS f, FuncSS df, double min_t, double max_t, double init_t )
{
	...
	double t = init_t;
	double q = 0.0;
	iterCount = kmb::Optimization::maxIter;
	while( iterCount > 0 ){
		q = df(t0);
		if( fabs(q) < kmb::Optimization::epsilon ){
			break;
		}
		residual = f(t0) / q;
		t = t0 - residual;
...
kmb::Optimization::calcZero_NR( OptTargetVV &obj, double* opt_t, const double* min_t, const double* max_t, const double* init_t )
{
	...
	iterCount = kmb::Optimization::maxIter;
	while( iterCount > 0 ){
		if( !obj.f(t0,q) || !obj.df( t0, h ) || fabs(h.determinant()) < kmb::Optimization::epsilon ){
			break;
		}
		if( h.solve(q,s) ){
			t.setDifference(t0,s);
			residual = t.distanceSq(t0);
			if( residual < kmb::Optimization::epsilon*kmb::Optimization::epsilon ){
				break;
			}
			for(int i=0;i<dim;++i){
...
kmb::Optimization::calcZero_DN( OptTargetSS &obj, double init_t )
{
	...
	while( iterCount > 0 ){
		residual = 2.0;
		y = obj.f(t0);
		if( fabs(y) < kmb::Optimization::epsilon ){
			return t0;
		}
		d = obj.df(t0);
		if( fabs(d) < kmb::Optimization::epsilon ){
			break;
		}
		d = - y / d;
		do{
			residual *= 0.5;
			t = t0 + residual * d;
		}while( fabs(obj.f(t)) > ( 1 - 0.5*residual ) * fabs(y) );
		t0 = t;
		--iterCount;
	}
	return DBL_MAX;
...
kmb::Optimization::calcZero_DN( OptTargetVV &obj, double* opt_t, const double* init_t )
{
	...
	while( iterCount > 0 ){
		residual = 2.0;
		obj.f(t0,y0);
		if( y0.normSq() < kmb::Optimization::epsilon*kmb::Optimization::epsilon ){
			res = true;
			for(int i=0;i<dim;++i){
				opt_t[i] = t0[i];
			}
		...
		do{
			residual *= 0.5;
			for(int i=0;i<dim;++i){
				t[i] = t0[i] - residual * d[i];
			}
			obj.f(t,y);
		}while( y.normSq() > ( 1 - 0.5*residual ) * ( 1 - 0.5*residual ) * y0.normSq() );
		t0 = t;
		--iterCount;
	}
	return res;
...
kmb::Optimization::calcZero_secant( OptTargetSS_0 &obj, double init_t0, double init_t1 )
{
	...
	iterCount = kmb::Optimization::maxIter;
	while( iterCount > 0 ){
		if( fabs(q1) < kmb::Optimization::epsilon ){
			break;
		}
...
bool
kmb::Optimization::getMin_BFGS( OptTargetSV &obj, double* opt_t, double* init_t )
{
	if( opt_t == NULL || init_t == NULL ){
		return false;
	}
	...
	int count = kmb::Optimization::maxIter;
	while( count > 0 ){
		...
		}while( obj.f(t) > y + 0.5 * alpha * v );

		obj.df( t, d );
		if( kmb::ColumnVector::normSq( d, dim ) < kmb::Optimization::epsilon * kmb::Optimization::epsilon ){
			for(int i=0;i<dim;++i){
				opt_t[i] = t[i];
			}
			res = true;
...
kmb::Optimization::calcMin_GS( FuncSS f, double min_t, double max_t )
{
	...
	while( iterCount > 0 ){
		residual = t1 - t0;
		if( residual < kmb::Optimization::epsilon ){
			break;
		}
...
kmb::Optimization::calcMin_GS( OptTargetSV_0 &obj, double* opt_t, const double* min_t, const double* max_t )
{
	...
	while( iterCount > 0 ){
		...
		for(int i=0;i<n;++i){
			residual += fabs(t0[i]-t1[i]);
		}
		if( residual < n*kmb::Optimization::epsilon ){
			break;
		}
...
bool
kmb::Optimization::getMin_CG( OptTargetSV &obj, double* opt_t, const double* min_t, const double* max_t, const double* init_t )
{
	if( opt_t == NULL || min_t == NULL || max_t == NULL ){
		return false;
	}
	...
	sub_obj.t = t;
	sub_obj.conj = conj;
	double epsilonSq = kmb::Optimization::epsilon * kmb::Optimization::epsilon;
	int count = kmb::Optimization::maxIter;
	double lenSq = 0.0;
